...
def sig_cost(cm,fa,sig,detectable_faults,show=False):
    ...
    total_cost=C+P
    if show:
        print('   [D] Total costs: '+str(total_cost)+' | '+'Penalty: '+str(penalty)+' | '+'P: '+str(P))
    return total_cost
...
def ga_search(cm,fault_activations,detectable_faults,nP=500,nI=500,pC=1,mu_o=0.05):
    #seeds=
    mu=mu_o
    mso_size=len(cm)
    cm=np.array(cm)
    ...
    for i in range(nP):
        ...
        for m in mu_list:
            if sig_P[m]==1:
                sig_P[m]=0
        cost_P=sig_cost(cm,fault_activations,sig_P,detectable_faults)
    ...
    for i in range(nI):
        ...
        if (i%100)==0:
            ...
            sig_cost(cm,fault_activations,best_ever['signature'],detectable_faults,show=True)
...
def find_set_v2(fault_manager):
    ...
    best_ever = ga_search(theta,fault_activations,detectable_faults)
    return list(np.where(best_ever['signature']==1)[0])#theta,fault_activations,detectable_faults,theta,best_ever      
    
            
"""theta, fault_activations, detectable_faults, cm,best_ever = find_set_v2(fault_manager)       

sig=[0,13,39,44,49,50,94,110,121,122,126,128,141,149]
sig=[0,3,6,54,77,102,169]
s=[]                   
for i in range(186):
    if i in sig:
        s.append(1)
...
new_fa={}
for f in detectable_faults:
    new_fa[f]=[]
    for i in sig:
        if i in fault_activations[f]:
            new_fa[f].append(i)

sig_cost(cm,fault_activations,s,detectable_faults,show=True)"""
