...
def set_new_model(mso_path,host,machine,matrix,sensors_in_tables,faults,sensors,sensor_eqs,preferent,time_bands,filt_val,filt_param,filt_delay_cap,main_ca,max_ca_jump,cont_cond,spec_list,retrain=False,aggSeconds=5,sam=100000,version="",mso_set=[],production=False):
    ...
    if do:
        ...
        #print(faults)
        fault_manager = fault_detector(file,mso_path,host,machine,matrix,sensors_in_tables,faults,sensors,sensor_eqs,preferent,filt_val,filt_param,filt_delay_cap,main_ca,max_ca_jump,aggS=aggSeconds)
        ...
        if abs(sam-fault_manager.training_data.shape[0])>10:
            ...
            fault_manager.test_data=rest.sample(n=int(sam/6))
            fault_manager.kde_data=homo_sampling(filt_data,cont_cond,s=int(sam/6))
        ...
        response_dic=fault_manager.train_residuals(folder,file,spec_list,predictor='NN',outlayers='No')
