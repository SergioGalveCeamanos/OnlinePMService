...
class residual:
     def __init__(self,mso_index,mso_reduced_index,sensed_vars,variables,faults,equations,sensor_names):
...
#### LASTM in TF: https://towardsdatascience.com/lstm-by-example-using-tensorflow-feb0c1968537
         
# NN Model created with size proportional to the # of variables
     def build_model_nn(self,shape,size): # best result in models.txt
         if size>2:
             i=-1
             list_layers=[]
             new_l=[]
             layer_list=self.spec_list['layers']
             adam=self.spec_list['adam']
             First=True
             while i<(len(layer_list)-1):
                 i=i+1
                 if layer_list[i]['Layer']=='L':
                     if First:
                         First=False
                         new_l=layers.Dense(round((layer_list[i]['k']*size)), activation=tf.nn.relu, input_shape=[shape])
                     else:
                         new_l=layers.Dense(round((layer_list[i]['k']*size)), activation=tf.nn.relu)
                 elif layer_list[i]['Layer']=='LR':
                     if First:
                         First=False
                         new_l=layers.Dense(round((layer_list[i]['k']*size)), activation=tf.nn.relu, input_shape=[shape],kernel_regularizer = keras.regularizers.l2(layer_list[i]['l2']))
                     else:
                         new_l=layers.Dense(round((layer_list[i]['k']*size)), activation=tf.nn.relu,kernel_regularizer = keras.regularizers.l2(layer_list[i]['l2']))
                 elif layer_list[i]['Layer']=='D':
                     new_l=layers.Dropout(layer_list[i]['d'])
                 list_layers.append(new_l)   
             list_layers.append(layers.Dense(size+1, activation=tf.nn.relu))
             list_layers.append(layers.Dense(size+1, activation=tf.nn.relu))
             list_layers.append(layers.Dense(1))
             self.model = keras.Sequential(list_layers)  
             ...
             self.model = keras.Sequential([
                 layers.Dense(round((3)), activation=tf.nn.relu, input_shape=[shape]),
     ...
     def build_model_lstm(self,data,size):
         #https://towardsdatascience.com/choosing-the-right-hyperparameters-for-a-simple-lstm-using-keras-f8e9ed76f046
         self.model = tf.keras.models.Sequential()
         self.model.add(tf.keras.layers.LSTM(32*size,input_shape=data.shape[-2:]))
...
###################################### Training ################################################
     ...
     def train(self,data,validation,kde_data,source,objective,predictor='NN',option2='PCA',acceptance=2):  
         ...
         for name in list(data.columns):
             if (name not in source) and (name!=objective) and (name not in self.kde_dims):
         ...
         #data.plot(linewidth=0.35)
         #plt.show()
         for name in list(data.columns):
             if (name not in source) and (name!=objective):
                 data=data.drop([name],axis=1)
                 validation=validation.drop([name],axis=1)
                 kde_data=kde_data.drop([name],axis=1)
         #train_proportion=0.8
         """if predictor=='NN':
             #create the apropiate dataset
             train_dataset = data.sample(frac=train_proportion,random_state=np.random.RandomState(1234))
             test_dataset = data.drop(train_dataset.index)  
             saved_test = original.drop(train_dataset.index)
         elif predictor=='LSTM':
             #how can we split it in a better way?
             index=data.index
             train_dataset = data.loc[index[0:round(train_proportion*data.shape[0])]]
             test_dataset = data.drop(train_dataset.index)"""
         ...
         train_labels = data.pop(objective)
         test_labels = validation.pop(objective)
         kde_labels = kde_data.pop(objective)
         ...
         if predictor=='NN':
         # Build the TF parameters and commands
             self.shape_nn=len(data.keys())
             early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)
             self.build_model_nn(self.shape_nn,len(source))
             train_labels,normed_train_data=self.dropna(train_labels,normed_train_data)
             test_labels,normed_test_data=self.dropna(test_labels,normed_test_data)
             print('Training Data conversion to Tensor:')
             print('     Train Labels --> Type: '+str(type(train_labels))+', shape: '+str(train_labels.shape))
             print('     Train Data --> Type: '+str(type(normed_train_data))+', shape: '+str(normed_train_data.shape))
             #tl=tf.convert_to_tensor(train_labels)
             #ntd=tf.convert_to_tensor(normed_train_data)
             history = self.model.fit(normed_train_data, train_labels, epochs=self.epochs,validation_data = (normed_test_data,test_labels), verbose=0, callbacks=[early_stop, PrintDot()])
             #self.plot_history(history)
             
         # OBSOLETE !!!!!!!!!!!!!!!!!!!!!    
         elif predictor=='LSTM':
             #what to do with data so that it is in order?
             ...
             future_target = 0
             STEP = 1
             index_unc=uncertainty_vars.index
             uncertainty_vars=uncertainty_vars.loc[index_unc[past_history:]]
             x_train_single, y_train_single = self.multivariate_data(normed_train_data, train_labels,0, None, past_history,future_target, STEP,single_step=True)
             normed_test_data, test_labels = self.multivariate_data(normed_test_data, test_labels,0, None, past_history,future_target, STEP,single_step=True)
             ...
             #normed_test_data= val_data_single
             self.build_model_lstm(x_train_single,len(source))
             history = self.model.fit(train_data_single, epochs=EPOCHS,steps_per_epoch=EVALUATION_INTERVAL,validation_data=val_data_single,validation_steps=50)
         ...
         kde_train = self.model.predict(normed_kde).flatten()
         # TRY NEW FUNCTION FOR DATA VALIDATION
         if predictor=='LSTM':
             test_labels=np.array(test_labels[:,0])
         
         error = test_predictions - test_labels
         error_kde = kde_train - kde_labels
         x=[]
         ...
         plt.title(("Errors for MSO # "+str(self.mso_reduced_index)))
         plt.ylabel("Count")
         plt.show()   
         
         fig, ax = plt.subplots()
         ax.plot(x[:500],test_predictions[:500],'r',linewidth=0.35,label='Predicted Values')
         ax.plot(x[:500],test_labels[:500],'b',linewidth=0.35,label='Measured Values')
         plt.title(("Prediction MSO #"+str(self.mso_reduced_index)))
         plt.xlabel('Samples')
         ...
         if acceptance*np.std(error)>self.train_stats.loc[objective,'std']:
             ...
             self.model_uncertainty(error_kde,kde_data,option=option2)
     ...
     def model_uncertainty(self,errors,variables,option='default'):
         kde_data,plt_names=self.kde_gathering(errors,variables,option)
         print(kde_data)
         #is it ok to get the std from such small subsample?
         train_stats = kde_data.describe()
         self.kde_stats = train_stats.transpose()
         normed_data = self.norm(kde_data,self.kde_stats)
         normed_data=normed_data.dropna()
         #kde_data.plot.kde()
         grid = GridSearchCV(KernelDensity(),
                    {'bandwidth': np.linspace(0.01, 1, 20)},
                    cv=3) # 20-fold cross-validation
         grid.fit(normed_data.values)
         
         #print(grid.best_params_)
         # construct a kernel density estimate of the distribution
         self.kde = KernelDensity(bandwidth=grid.best_params_['bandwidth'], metric='euclidean',kernel='gaussian', algorithm='ball_tree')
         self.kde.fit(normed_data.values)
         ...
         #np.linspace(kde_data[self.kde_dims[0]][mins[self.kde_dims[0]]],kde_data[self.kde_dims[0]][maxs[self.kde_dims[0]]],100)
         #maxs=kde_data.idxmax()
         #mins=kde_data.idxmin()
         #normed_data.plot(linewidth=0.35)
         #plt.show()
         self.kde_std=normed_data.std()
         self.kde_mean=normed_data.mean()
         """m_y=[(self.kde_mean['Errors']-1*self.kde_std['Errors']),(self.kde_mean['Errors']+1*self.kde_std['Errors'])]
         if len(self.kde_dims)>1:
             # to consider a 95%, in the plot at least ... more necessary?? (Based on the normal distribution)
             m_1=[(self.kde_mean[plt_names[0]]-1*self.kde_std[plt_names[0]]),(self.kde_mean[plt_names[0]]+1*self.kde_std[plt_names[0]])]
             m_2=[(self.kde_mean[plt_names[1]]-1*self.kde_std[plt_names[1]]),(self.kde_mean[plt_names[1]]+1*self.kde_std[plt_names[1]])]
             for y in np.linspace(m_y[0],m_y[1],100):
                 for x1 in np.linspace(m_1[0],m_1[1],100):
                     data1.append([y,x1,normed_data.mean()[plt_names[1]]]) #kde_data.mean()[self.kde_dims[1]]
                     x_1.append(x1)
                     y_1.append(y)
                 for x2 in np.linspace(m_2[0],m_2[1],100):
                     data2.append([y,normed_data.mean()[plt_names[0]],x2])
                     x_2.append(x2)
                     y_2.append(y)
             
             Z1=np.exp(self.kde.score_samples(data1))
             Z2=np.exp(self.kde.score_samples(data2))
             gridsize=30
             
             '''PLT.subplot(111)
             PLT.hexbin(x_1, y_1, C=Z1, gridsize=gridsize, cmap=CM.jet, bins=None)
             ...
             PLT.subplot(111)
             PLT.hexbin(x_2, y_2, C=Z2, gridsize=gridsize, cmap=CM.jet, bins=None)
             ...
             for y in np.linspace(m_y[0],m_y[1],100):
                 for x1 in np.linspace(m_1[0],m_1[1],100):
                     data1.append([y,x1]) #kde_data.mean()[self.kde_dims[1]]
                     x_1.append(x1)
                     y_1.append(y)
             ...
             cb = PLT.colorbar()
             cb.set_label('mean value')
             plt.title(("PDF Heatmap MSO #"+str(self.mso_reduced_index)))
             plt.xlabel(('Normalized '+plt_names[0]))
     ...
     def predict(self,new_data,plot='No'):
         measured_value=new_data[self.objective]
         ...
         error = predictions - measured_value
         if plot=='Yes':
             x=[]
             ...
             ax.plot(x[:600],measured_value[:600],'b',linewidth=0.35,label='Measured Values')
             plt.title(("Prediction MSO #"+str(self.mso_reduced_index)))
             plt.xlabel('Samples')
             plt.ylabel(self.objective)           
             plt.legend()
             plt.show()  '''
         return error
     def sample_score(self,variable_set,m_y,sensor_acc,sample_n,plt_names,low,high,errors,alpha,phi,avoid,conf_factor=0.01,samples=[300,50]):
         label_num=-1
         for sample in variable_set:
             ...
             try:
                 ...
                 new_measure=[]
                 new_max=[]
                 for y in np.linspace(m_y[0],m_y[1],samples[0]):
                     n=[((y-self.kde_stats.loc["Errors","mean"])/self.kde_stats.loc["Errors","std"])]
                     i=0
                     for name in plt_names:
                         n.append(((sample[i]-self.kde_stats.loc[name,"mean"])/self.kde_stats.loc[name,"std"]))
                         i=i+1
                     new_cut.append(n)
                 Z=np.exp(self.kde.score_samples(new_cut))
                 mp=new_cut[np.where(Z == max(Z))[0][0]][0]
                 max_point=[((mp*self.kde_stats.loc["Errors","std"])+self.kde_stats.loc["Errors","mean"])]
                 #print(max_point)
                 integral=sum(Z)
                 if integral>0:
                     P=Z/integral
                     #second get the sensor accuracy area
                     area=[(sample[0]-sensor_acc*self.kde_stats.loc["Errors","std"]),(sample[0]+sensor_acc*self.kde_stats.loc["Errors","std"])]
                     ...
                     area_max=[(max_point-sensor_acc*self.kde_stats.loc["Errors","std"]),(max_point+sensor_acc*self.kde_stats.loc["Errors","std"])]
                     for y in np.linspace(area_max[0],area_max[1],samples[1]):
                         n_max=[((y-self.kde_stats.loc["Errors","mean"])/self.kde_stats.loc["Errors","std"])]
                         i=0
                         for name in plt_names:
                             n_max.append(((sample[i]-self.kde_stats.loc[name,"mean"])/self.kde_stats.loc[name,"std"]))
                             i=i+1
                         new_max.append(n_max)
                     #assume width 1 to simplify the integral of the full fampling and scale for the other
                     width_full=(m_y[1]-m_y[0])/samples[0]
                     width_measure=(area[1]-area[0])/samples[1]
                     Z_m=np.exp(self.kde.score_samples(new_measure))
                     integral_measure=sum(Z_m)*width_measure/width_full
                     Z_max=np.exp(self.kde.score_samples(new_max))
                     ...
                     for j in range((len(P)-1)):
                         ...
                         if acc<conf_factor and (acc+P[j+1])>conf_factor:
                             nlow=(new_cut[j][0]*self.kde_stats.loc["Errors","std"]+self.kde_stats.loc["Errors","mean"])
                             low[str(label_num+sample_n)]=nlow
                         if acc<(1-conf_factor) and (acc+P[j+1])>(1-conf_factor):
                             nhigh=(new_cut[j][0]*self.kde_stats.loc["Errors","std"]+self.kde_stats.loc["Errors","mean"])
     ...
     def score(self,variables,dic_fill,plt_names,times,option='default',std=5):
         m_y=[(self.kde_stats.loc["Errors","mean"]-std*self.kde_stats.loc["Errors","std"]),(self.kde_stats.loc["Errors","mean"]+std*self.kde_stats.loc["Errors","std"])]
         ...
         for i in range(proc):
             ...
             p = multiprocessing.Process(target=self.sample_score, args=(variables[sample_start:sample_stop],m_y,sensor_acc,sample_n,plt_names,low,high,errors,alpha,phi,avoid))
             p.start()
             jobs.append(p)
     ...
     def evaluate_performance(self,data,option1='NN',option2='PCA'):
         ...
         normed_kde = self.norm(kde_data,self.kde_stats)
         kde_arr=[]
         i=-1
         ...
         t_a=datetime.datetime.now()
         dic_fill={}
         forget=self.score(kde_arr,dic_fill,plt_names,times)
         t_b=datetime.datetime.now()
         dif=t_b-t_a
...
class fault_detector:
     def __init__(self,filename,mso_txt,host,machine,matrix,sensors,faults,sensors_lookup,sensor_eqs,preferent,filt_value,filt_parameter,filt_delay_cap,main_ca,max_ca_jump,aggS=5):
...
###################################### Get Residuals #######################################
     ...
     def MSO_residuals(self):
         ...
         for mso in self.msos:
             ...
             if known not in self.residuals_id:
                 ...
                 self.models.append(residual(n,o,known,variables,faults,equations,self.sensors))
             else:
                 repeated=repeated+1
     ...
     def train_mso(self,mso,predictor,option2,folder,return_dic,spec_list):
         ...
         while not_done:
             ...
             to_train=residual(n,o,known,variables,faults,equations,self.sensors)
             to_train.spec_list=spec_list
             print('[D] Inside train mso function for mso #'+str(mso))
             return_dic[self.get_dic_entry(mso)]=to_train.train(self.training_data,self.test_data,self.kde_data,source,goal,predictor,option2)
     ...
     def train_residuals(self,folder,file,spec_list,predictor='NN',outlayers='No',option2='PCA'):
         manager = multiprocessing.Manager()
         ...
         for mso in self.mso_set: 
             ...
             p = multiprocessing.Process(target=self.train_mso, args=(mso,predictor,option2,folder,return_dic,spec_list))
             p.start()
             jobs.append(p)
         for proc in jobs:
             proc.join()
